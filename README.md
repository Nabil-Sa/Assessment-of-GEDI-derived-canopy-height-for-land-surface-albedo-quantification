...
Where the azimuthal integral is omitted because the theoretical athmospheric conditions in which WSA exists are azimuthally symmetric as previously mentioned. Regarding the field of optical remote sensing, it differentiates between passive and active means, where passive remote sensing implies inference from an external source like the sun, whereas active remote sensing techniques as seen in Synthetic Aperture Radar (SAR) and Light Detection And Ranging (LiDAR) are able to operate independently of the external source (Verhoef, 1998). Regarding active remote sensing, space-borne LiDAR offers the means to synthesize and interpret the three-dimensional structural makeup of a target to derive biophysical parameters. LiDAR is broadly categorised as utilizing either pulsed ranging or continuous wave (CW) as the operating mode, where pulsed ranging is currently still the most widely used, which can be attributed to several reasons such as its high penetration capabilities over long distances and its lower sensitivity to electromagnetic degradation (Wang et al, 2024). Furthermore, when deriving the three-dimensional structural makeup with LiDAR, there is a distinction between full waveform sampling and discreet sampling in the amplitude of energy distribution, where full waveform sampling measures the amplitude of energy continuously, while discreet sampling records some points above a predefined threshold of returning energy amplitude, leading to a waveform profile less indicative of the canopy structure, though both methods are able to derive the canopy height (Lim et al, 2003). The photon attenuation of traveled distance through the canopy is a continuous process that results in an exponential decay curve of available photons and is thus widely captured by the Beer-Lambert law, though in an ecological and satellite processing context this is commonly referred to as the Monsi-Saeki theory (Hirose, 2004, Homma et al, 2025, Melnikova et al, 2018). The Monsi-Saeki theory was the first mathematical model to capture canopy photosynthesis based on photon attenuation (Monsi & Saeki, 1953). This theory established that light intensity decreases exponentially as it travels through a given canopy, also finding that most typical forest canopies block 80% to 98% of incoming sunlight. When discussing the gap theory and mechanics of the GEDI instrument, the formal name, Lambert-Beer's law, will be used in conjunction with and followed by Monsi-Saeki for clarity. 


### 2. Materials and Methods

#### 2.1. GEDI-derived canopy metrics
Global Ecosystem Dynamics Investigation (GEDI) was launched in december 2018 as a result of a joint collaboration between the University of Maryland and the NASA Goddard Space Flight Center with one of the main aims being the deepening of understanding terrestrial ecology with LiDAR observations through 3 lasers and detailed spatial sampling between ~52° N and ~52° S in order to fill observation gaps by inferring the three-dimensional structure of the earth's surface as mentioned in the Algorithm Theoretical Basis Document (ATBD) of the L2B product by (Tang & Armston, 2019). Furthermore, the instrument is able to derive various geophysical parameters related to forest structure and canopy cover. Canopy cover is described as the proportion of a given horizontal surface obscured by the vertical projection of the canopy (Stojanova et al, 2010). The Leaf Area Index (LAI) shares a relationship with the Canopy Cover Fraction (CCF) because they both depend on the gap probability of a canopy at a given angle. Here, $$\text{P}(\theta)$$ represents the probability of light penetrating a canopy without interference at angle $$(\theta)$$ (Chen & Cihlar, 1995). The returned laser energy of a waveform LiDAR system is normally dependent on environmental variables and canopy-related characteristics, implying that solving for the returned energy would require accounting for these variables. The ATBD incorporates the work of (Ni-Meister et al. 2001), which introduced an approach to solve for the gap probability of a given canopy without needing to perform any type of calibration to account for environmental variables. This idea assumes that a laser photon's interaction with the canopy is largely binary enough to not significantly distort the waveform profile, where it can either hit a leaf in a given layer or penetrate through physical openings and reach the ground. Thus, the rate of change of returned canopy energy and ground return energy would be sufficient to derive gap probability. In the ATBD, the returned canopy and ground energy are expressed as: 

$$-\frac{dR_v(z)}{dz} = J_0 \ \rho_v \ \frac{dP_{\text{gap}}(z,\theta)}{dz / \cos\theta}$$

$$R_g = J_0 \ \rho_g \ P_{\text{gap}}(0, \theta) \cos\theta$$

where $J_0$ represents the emitted irradiance, $$\rho_v\$$ is a given leaf's scattering coefficient, $dP_{\text{gap}}$ represents the differential in gap fraction across an infinitesimal height layer which is represented by $dz$. like $\rho_v$, $\rho_g$ represents the understory's reflectivity and $\cos\theta$ accounts for off-nadir tilt.

In the ATBD, using Beer's law (Monsi-Saeki formulation), this is expressed as:

$$P(\theta) = 1 - \text{Cover}(\theta) = e^{-\frac{G(\theta) \cdot \Omega(\theta) \cdot \text{PAI}}{\cos(\theta)}}$$

Where $G(\theta)$ represents the foliage projection coefficient that accounts for the specific ambiguous projected area of leaves at angle $\theta$. This gap theory is further exploited by differentiating between the gap probabilities at different height increments ($\Delta z$) above ground level relative to the tree canopy expressed as:

$$\ P_{gap}(z + \Delta z, \theta) = P_{gap}(z, \theta) \cdot \(1 - \frac{F_{app}(z) \cdot \Delta z}{\cos \theta}\)$$

where $P_{\text{gap}}(z + \Delta z, \theta)$ accounts for the gap probability above the current layer, $P_{gap}(z, \theta)$ represents the probability of remaining light exiting the layer, and $F_{\text{app}}(z)$ represents the known foliage profile incrementally intercepting light at height $z$. This foliage density profile is derived from:

$$
F_{\text{app}}(z) = \lim_{\Delta z \to 0} -\frac{P_{\text{gap}}(z+\Delta z, \theta) - P_{\text{gap}}(z, \theta)}{P_{\text{gap}}(z, \theta) \cdot \left(\frac{\Delta z}{\cos \theta}\right)} = \lim_{\Delta z \to 0} -\frac{\Delta P_{\text{gap}}(z, \theta)}{P_{\text{gap}}(z, \theta) \cdot \left(\frac{\Delta z}{\cos \theta}\right)} = - \frac{d \ln \left\lbrace P_{\text{gap}}(z, \theta) \right\rbrace}{dz / \cos \theta}
$$

where $${d \ln \left\lbrace P_{\text{gap}}(z, \theta) \right\rbrace}$$ decouples $$F_{\text{app}}(z)$$ from the exponential light decay curve.


#### 2.2 Kernel-driven BRDF
